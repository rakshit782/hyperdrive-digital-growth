...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (!savedServices) {
      console.log("Dashboard: Initializing services in localStorage");
      ...
      try {
        ...
        console.error("Failed to parse services data:", error);
        localStorage.setItem('servicesData', JSON.stringify(services));
        window.dispatchEvent(new CustomEvent('servicesUpdated', { detail: services }));
      }
    }
    ...
    if (!savedReviews) {
      console.log("Dashboard: Initializing reviews in localStorage");
