...
const PrivacyPolicy = () => {
  ...
  const policyContent = getPolicyPageByType('privacy-policy');

  if (loading) {
    return (
      <>
        <SEOHead title="Privacy Policy" description="Privacy Policy for Your Agency" />
        <Header />
          ...
          <div className="max-w-4xl mx-auto px-6 py-16">
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl p-8 md:p-12">
              <div className="text-center">
                <p className="text-red-600">Failed to load privacy policy: {error || 'Content not found'}</p>
              </div>
            </div>
          </div>
        </div>
