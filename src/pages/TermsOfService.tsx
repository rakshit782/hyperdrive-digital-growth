...
const TermsOfService = () => {
  ...
  const policyContent = getPolicyPageByType('terms-of-service');

  if (loading) {
    return (
      <>
        <SEOHead title="Terms of Service" description="Terms of Service for Your Agency" />
        <Header />
          ...
          <div className="max-w-4xl mx-auto px-6 py-16">
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl p-8 md:p-12">
              <div className="text-center">
                <p className="text-red-600">Failed to load terms of service: {error || 'Content not found'}</p>
              </div>
            </div>
          </div>
        </div>
